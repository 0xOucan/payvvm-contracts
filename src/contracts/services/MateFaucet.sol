...
/**
 * @title MateFaucet - EVVM Service for Gasless MATE Distribution
 * @author PAYVVM Team
 * @notice Faucet service that distributes MATE tokens without requiring users to pay gas
 * @dev Uses EVVM's caPay() for gasless token distribution
 *
 * Key Features:
 * - Gasless MATE claims using EIP-191 signatures
 ...
 *
 * Flow:
 * 1. Owner funds faucet with MATE (deposits to EVVM)
 * 2. User signs claim message off-chain (no gas)
 * 3. Fisher executes claim transaction on-chain
 * 4. Faucet uses caPay() to transfer MATE to user
 */
...
contract MateFaucet {
    ...
    /// @notice MATE token address (EVVM principal token)
    address public immutable mateToken;

    /// @notice Owner address for admin operations
    address public owner;

    /// @notice Amount of MATE to distribute per claim (in token decimals)
    ...
    /**
     * @notice Initialize the MATE faucet service
     * @param _evvm EVVM contract address
     * @param _mateToken MATE token address (EVVM principal token: 0x0000...0001)
     * @param _owner Faucet owner address
     * @param _claimAmount Amount of MATE per claim (in token decimals, e.g., 510 * 1e18 for 510 MATE)
    ...
    /**
     * @notice Claim MATE from faucet using signature-based gasless transaction
     * @dev Called by fishers who execute the signed message on behalf of users
     *
     * Signature Message Format:
     * {evvmID},claimMate,{claimer},{nonce}
     *
     * Example: "1000,claimMate,0x1234...,42"
     *
     * @param claimer Address claiming the MATE
    ...
    function claimMate(
        ...
        uint256 faucetBalance = evvm.getBalance(address(this), mateToken);
        ...
        // 7. Transfer MATE using EVVM's caPay (gasless for user)
        evvm.caPay(claimer, mateToken, claimAmount);
    ...
    /**
     * @notice Get faucet's MATE balance in EVVM
     * @return Current MATE balance
     */
    function getFaucetBalance() external view returns (uint256) {
        return evvm.getBalance(address(this), mateToken);
    }
    ...
    /**
     * @notice Withdraw MATE from faucet (emergency or refund)
    ...
    function withdraw(address recipient, uint256 amount) external onlyOwner {
        ...
        evvm.caPay(recipient, mateToken, amount);
